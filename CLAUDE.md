...
- **Code**: one company Django app with its own label, its own `migrations/` directory and an `AppConfig.ready()` for signals and hook assertions. Its location is still an open decision (`dojo/company/` ships through the existing `COPY dojo/`; a top-level `company/` stays outside upstream meta-tests but needs a `COPY` in `Dockerfile.company`). Settle it before the first code lands and record it in `Fork Strategy.md`.
