...
- Base version 3.3.200 (weekly patches merged 2026-09-19 and 2026-09-22). Every upstream tag (monthly `x.y.0`, weekly `x.y.N00`) is merged into `company/main` as a real merge commit; see the release-merge checklist in the vault's `Fork Strategy.md`.
...
- **Containers**: `docker-compose.company.yml` layered with explicit `-f` flags (`COMPOSE_FILE` separators differ per OS: `:` on Linux, `;` on Windows). `Dockerfile.company` layers `requirements-company.txt` (django-auth-ldap, python-ldap) on the upstream image: build the upstream image first, then the company layer, both with the same `-f` set minus/plus the company file. Images are tagged `<version>-company`. The `ldap` profile starts a seeded OpenLDAP test directory (`docker/company/ldap/`).
- **Company profiles**: `companies/<name>/<name>.env` (non-secret `DD_COMPANY_*` values, loaded with `docker compose --env-file`) and `companies/<name>/assets/` (artwork). `bash docker/company/apply-profile.sh <name>` copies the artwork into the static tree for a build; never commit profile artwork into `dojo/static`, the tree stays neutral with placeholders. The release workflow takes a `profile` input and suffixes the image tags with it. A variable in a profile reaches the containers only when a compose file references it; `apply-profile.sh` and the `profiles` CI job refuse a profile that sets an unreferenced one. First profile: `zaincash`.
- **LDAP / Active Directory**: `dojo/company/ldap.py` (settings builder, group-to-product-type sync) and `dojo/company/ldap_backend.py` (backend, needs django-auth-ldap). Off unless `DD_COMPANY_LDAP_ENABLED=True`; `DD_COMPANY_LDAP_PROFILE` is `ad` or `openldap`; the admin group grants superuser and staff; groups named `<prefix><product type>` grant that product type and the directory is authoritative for LDAP users on every login. Local `ModelBackend` stays last. LDAPS against an internal CA: the CA PEM goes to `secrets/dd_company_ldap_ca_cert`, read through `DD_COMPANY_LDAP_CA_CERT_PATH` (a path, deliberately not a `_FILE` variable, which `docker/secret-file-loader.sh` would turn into file content). Workstation test: `DD_COMPANY_LDAP_ENABLED=True DD_COMPANY_LDAP_PROFILE=openldap DD_COMPANY_LDAP_BIND_PASSWORD=admin docker compose $F --profile ldap up -d` (seeded users alice/bob/carol).
- **Modules imported from `local_settings.py`** (like `dojo.company.ldap`) must not import models at module level: settings are still loading and the app registry does not exist yet (`AppRegistryNotReady`). Import models inside functions.
